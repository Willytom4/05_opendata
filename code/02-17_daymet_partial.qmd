...
field %>%
  distinct(year) %>%
  summarise(number_of_years = n_distinct(year))
```
...
states
#without filtering, we have too many wide spaces due to geographical spaces
# ! flips the statement. We state tat we want the 3 states but ! flips it to the other states. Now the map is clean without the many spaces
...
#separate the measurements by pivoting wider
```
How many rows above? Why?  
...
```{r}
daymet_all_unnest <- unnest(daymet_all,
                            cols = weather) %>%
  pivot_wider(names_from = measurement,
              values_from = value) %>%
  janitor::clean_names()
...
# daymet_all_unnest <- %>% dayment_all %>% unnest(weather) - used in class
# y = predicted or response variable while x = explanatory or predictor variable
daymet_all_unnest

```

How many rows? Why?  
7 variables for the year = 7 x 365 = 2555

weather has 698 for each of the 2555 data points has 698
2555 x 698 = 1783,390
# Exporting  
...
Let's do that below:  
```{r}
# we use ggsave for plots and write for other files
write_csv(daymet_all_unnest,
          "../output/fieldweatherdata.csv")
```
