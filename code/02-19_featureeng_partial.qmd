...
fe_month_sum_wide <- fe_month_sum %>% 
  pivot_longer(mean_dayl.s:sum_prcp.mm) %>% #to get all variables in a column
  mutate(varname = paste0(name, "_", month_abb)) %>% # to join two columns together without adding space
...
ggplot(data = fe_month_sum,
  ...
  geom_density_ridges_gradient(scale = 3,
                               rel_min_height = 0.01) + # helps to view the plots without overlapping
  scale_fill_viridis_c(option = "C", alpha = .8) + #Exam question Scale_fill because we used fill
